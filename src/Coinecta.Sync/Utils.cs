...
public static class Utils
{
    ...
    public static async Task<List<TransactionOutputEntity>> ResolveTransactionInputsAsync(CoinectaDbContext _dbContext, Block block)
    {
        List<string> txOutputRefs = block.TransactionBodies.SelectMany(tx => tx.Inputs
            .Select(txInput => txInput.Id.ToHex() + txInput.Index))
            .ToList();

        return await _dbContext.TransactionOutputs
            .AsNoTracking()
            .Where(transactioOutput => txOutputRefs.Contains(transactioOutput.Id + transactioOutput.Index))
            .Select(transactioOutput => transactioOutput)
            .ToListAsync();
    }
}